...
def basic():
    simple = []
    data = pd.read_csv("NFL Archives/nfl odds 2019-20.csv", delimiter=',')
    ...
    for file in glob.glob("NFL Archives/*.csv"):
        ...
        for i in range(0, len(data), 2):
            ...
            v_ml = v_teamdata['ML']
            h_ml = h_teamdata['ML']
            if v_teamdata['Close'] == "pk":
                close_ou = float(v_teamdata['Open'])
            else:
                close_ou = float(v_teamdata['Close'])
            over_ml = -110
            under_ml = -110
            #print(v_teamname, h_teamname)

            #St.Louis -> LARams
            if h_teamname == 'St.Louis' or h_teamname == 'LosAngeles':
                home = next((item for item in simple if item['team'] == 'LARams'))
            elif h_teamname == 'SanDiego':
                home = next((item for item in simple if item['team'] == 'LAChargers'))
            elif h_teamname == "HoustonTexans":
                home = next((item for item in simple if item['team'] == 'Houston'))
            elif h_teamname == "BuffaloBills":
                home = next((item for item in simple if item['team'] == 'Buffalo'))
            else:
                home = next(item for item in simple if item['team'] == h_teamname)

            if v_teamname == 'St.Louis' or v_teamname == 'LosAngeles':
                away = next((item for item in simple if item['team'] == 'LARams'))
            elif v_teamname == 'SanDiego':
                away = next((item for item in simple if item['team'] == 'LAChargers'))
            elif v_teamname == "HoustonTexans":
                away = next((item for item in simple if item['team'] == 'Houston'))
            elif v_teamname == "BuffaloBills":
                away = next((item for item in simple if item['team'] == 'Buffalo'))
            else:
    ...
    for team in simple:
        ...
        print(team['team'], team['v_over'], team['v_under'])
    print('\n')
